            ...
            <div class="card-body">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        ...
                        <input type="date" name="fecha_emision" value="{{ date('Y-m-d') }}" required class="form-control">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Forma de pago</label>
                        <select name="forma_pago" class="form-control">
                            @forelse(\App\Models\FormaPago::activos()->get() as $fp)
                            <option value="{{ $fp->clave }}">{{ $fp->etiqueta }}</option>
                            @empty
...
function seleccionarProveedor(p) {
    ...
    if (dias > 0) {
        badge.textContent = dias + ' días crédito (irá a CxP si PPD)';
...
async function buscarProductos(q) {
    try {
        const r = await fetch(`{{ route('compras.buscar-productos') }}?q=${encodeURIComponent(q)}`);
        const data = await r.json();
...
function agregarProducto(p) {
    ...
    productos.push({ id: p.id, codigo: p.codigo, nombre: p.nombre, cantidad: 1, precio: parseFloat(p.costo) || 0, descuento: 0, tasa_iva: p.tasa_iva != null ? p.tasa_iva : 0.16, manual: false });
