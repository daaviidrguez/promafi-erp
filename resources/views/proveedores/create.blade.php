...
@section('page-subtitle', 'Registrar proveedor para compras')

@php $breadcrumbs = [['title' => 'Proveedores', 'url' => route('proveedores.index')], ['title' => 'Nuevo']]; @endphp
        ...
        <div class="card-body">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div class="form-group">
                    ...
                    <input type="text" name="rfc" class="form-control text-mono" value="{{ old('rfc') }}" maxlength="13" style="text-transform:uppercase;">
                </div>
                <div class="form-group">
                    <label class="form-label">Días de crédito</label>
                    <input type="number" name="dias_credito" class="form-control" value="{{ old('dias_credito', 0) }}" min="0">
                </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="uso_cfdi" class="form-control">
                        ...
                        @foreach($usosCfdi ?? [] as $u)
                            <option value="{{ $u->clave }}" {{ old('uso_cfdi') == $u->clave ? 'selected' : '' }}>{{ $u->etiqueta }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
